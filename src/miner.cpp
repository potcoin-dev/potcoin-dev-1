...
// Copyright (c) 2017 The KekCoin developers
...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, uint64_t* pFees)
{
    ...
    if(!(fIncludeWitness || IsWitnessLocked(pindexPrev, chainparams.GetConsensus())))
...
void KekCoinStaker(const CChainParams& chainparams)
{
    LogPrintf("KekCoinStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kekcoin-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in KekCoinStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            LogPrint("coinstake","Running KekCoinStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("KekCoinStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("KekCoinStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            LogPrintf("KekCoinStaker: ProcessNewBlock() : %s\n", pblock->ToString());
            return error("KekCoinStaker: ProcessNewBlock, block not accepted");
        }
    }
